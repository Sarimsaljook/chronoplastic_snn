...
class SuperSpike(torch.autograd.Function):
    """Heaviside forward, triangular surrogate backward"""
    @staticmethod
...
class LongGapTwoCueDataset(Dataset):
    def __init__(self, num_samples=5000, T=100, C=16,
                 min_gap=6, max_gap=25, distractor_p=0.03, seed=0):
        super().__init__()
        g = torch.Generator().manual_seed(seed)
        self.X = torch.zeros(num_samples, T, C)
...
class BaselineSynapse(nn.Module):
    ...
    def forward(self, spikes_t, trace):
        trace_next = self.alpha.clamp(0,0.9999)*trace + spikes_t
        current = spikes_t@self.W + trace_next@self.W
        return current, trace_next


class ChronoPlasticSynapse(nn.Module):
    def __init__(self, c_in, c_out,
                 alpha_fast=0.90, alpha_slow=0.995, beta_ctrl_noise=0.1):
        super().__init__()
        self.W = nn.Parameter(torch.empty(c_in, c_out))
        nn.init.kaiming_uniform_(self.W, a=math.sqrt(5))
        ...
        self.beta_ctrl_noise = beta_ctrl_noise
        hidden = max(8, c_in//2)
        self.ctrl = nn.Sequential(
            nn.Linear(c_in*2, hidden), nn.ReLU(), nn.Linear(hidden, c_in)
        )
    def forward(self, spikes_t, fast, slow, return_warp=False):
        af = self.alpha_fast.clamp(0,0.9999)
        fast_next = af*fast + spikes_t
        ctrl_in = torch.cat([spikes_t, slow],1)
        ctrl_logits = self.ctrl(ctrl_in)
        if self.training and self.beta_ctrl_noise>0:
            ctrl_logits += self.beta_ctrl_noise*torch.randn_like(ctrl_logits)
        ...
        slow_next = alpha_adapt*slow + spikes_t
        cur = spikes_t@self.W + (self.gamma_fast*fast_next)@self.W \
              + (self.gamma_slow*slow_next)@self.W
        if return_warp: return cur, fast_next, slow_next, warp
...
class LIFLayer(nn.Module):
    def __init__(self, c_in, c_out, lif:LIFParams, chrono=True):
        super().__init__()
        self.c_in,self.c_out=c_in,c_out; self.v_th=lif.v_th; self.v_reset=lif.v_reset
        self.alpha=math.exp(-lif.dt/lif.tau_mem)
        self.chrono=chrono
        self.syn=ChronoPlasticSynapse(c_in,c_out) if chrono else BaselineSynapse(c_in,c_out)
    def forward(self, spikes_in, log=False):
        T,B,_=spikes_in.shape
        device=spikes_in.device
        v=torch.zeros(B,self.c_out,device=device)
        outs=[]; warps=[]
        if self.chrono:
            fast=torch.zeros(B,self.c_in,device=device)
            slow=torch.zeros(B,self.c_in,device=device)
            for t in range(T):
                if log:
                    cur,fast,slow,warp=self.syn(spikes_in[t],fast,slow,True)
                    warps.append(warp.mean().item())
                else:
                    cur,fast,slow=self.syn(spikes_in[t],fast,slow)
                v=self.alpha*v+(1-self.alpha)*cur
                s=spike_fn(v-self.v_th,75.0)
                v=torch.where(s>0,torch.full_like(v,self.v_reset),v)
                outs.append(s)
        else:
            trace=torch.zeros(B,self.c_in,device=device)
            for t in range(T):
                cur,trace=self.syn(spikes_in[t],trace)
                v=self.alpha*v+(1-self.alpha)*cur
                s=spike_fn(v-self.v_th,75.0)
                v=torch.where(s>0,torch.full_like(v,self.v_reset),v)
                outs.append(s)
...
class SpikingNet(nn.Module):
    def __init__(self,C_in,C_hidden,C_out,lif:LIFParams,chrono=True):
        ...
        self.chrono=chrono
    def forward(self,x_seq,log=False):
        s1,r1,w1=self.l1(x_seq,log)
        s2,r2,w2=self.l2(s1)
...
def main():
    ...
    set_seed(0); device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[INFO] device={device} hidden={args.hidden} epochs={args.epochs} lr={args.lr}")

    lif=LIFParams()
    C=16
    baseline=SpikingNet(C,C_hidden=args.hidden,C_out=args.hidden,lif=lif,chrono=False).to(device)
    cpsnn=SpikingNet(C,C_hidden=args.hidden,C_out=args.hidden,lif=lif,chrono=True).to(device)
    ...
    for ep in range(1,args.epochs+1):
        gap_min, gap_max = 4+ep//3, 10+ep//2
        noise = 0.002*ep
        ...
        acc_b=run_epoch(baseline,train_loader,opt_b,device,True)
        acc_c=run_epoch(cpsnn,train_loader,opt_c,device,True)
        val_b=run_epoch(baseline,val_loader,opt_b,device,False)
        val_c=run_epoch(cpsnn,val_loader,opt_c,device,False)
        acc_base.append(val_b); acc_cp.append(val_c)
        print(f"Ep{ep:02d} gap=({gap_min},{gap_max}) noise={noise:.3f} | "
              f"Base {val_b:.3f}  CPSNN {val_c:.3f}")

    # Plot accuracy curves
    plt.figure(figsize=(7,4))
    plt.plot(acc_base,label="Baseline SNN",linewidth=2)
    plt.plot(acc_cp,label="ChronoPlastic SNN",linewidth=2)
    plt.xlabel("Epoch"); plt.ylabel("Validation Accuracy")
    plt.title("CPSNN Learns Longer Temporal Dependencies")
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.show()

    # Visualize warp factor trend for one sequence
    sample,_=val_ds[0]; X=sample.unsqueeze(1).to(device)
    _,warps=cpsnn(X,log=True)
    plt.figure(figsize=(6,3))
    plt.plot(warps,label="Mean Warp Factor over Time")
    plt.xlabel("Timestep"); plt.ylabel("Warp Factor")
    plt.title("Adaptive Decay Controller Response (Cue Memory Strength)")
